...
class ResponseChannelBridgeSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  private def driveResp(dut: ResponseChannelBridge, channel: Int, bankId: Int, data: Int, valid: Boolean = true, regAddr: Int = 0): Unit = {
  ...
  private def runGroupedBridgeRoutingTest(respChannelCount: Int, banksToSend: Seq[Int]): Unit = {
    test(new ResponseChannelBridge(
      ...
      banksToSend.zipWithIndex.foreach {
        case (bankId, ch) =>
          dut.io.out(bankId).valid.expect(true.B, s"bank $bankId should receive its response in $respChannelCount-channel mode")
