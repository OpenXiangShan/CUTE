...
class CUTE2TLImp(outer: Cute2TL) extends LazyModuleImp(outer) with CUTEImplParameters{
  ...
  val mask = io.mmu.Request.bits.RequestMask
  ...
  tl_out.a.bits := Mux1H(Seq(
    ...
    (io.mmu.Request.bits.RequestType_isWrite === 1.U) -> edge.Put(id, io.mmu.Request.bits.RequestPhysicalAddr, log2Ceil(outsideDataWidthByte).U, data, mask)._2
  ))
