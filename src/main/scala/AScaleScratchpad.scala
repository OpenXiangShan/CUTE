...
class AScaleScratchpadIO(implicit p: Parameters) extends CuteBundle{
    val FromScaleController = new AScaleControlScaratchpadIO
    val FromScaleLoader = new AScaleLoaderScaratchpadIO 
}

class AScaleScratchpad(implicit p: Parameters) extends CuteModule{
    val io = IO(new Bundle{
        // val ConfigInfo = Flipped(DecoupledIO(new ConfigInfoIO))
        val ScaleScratchpadIO = new AScaleScratchpadIO
    })
    ...
    val bank = SyncReadMem(AScaleBankNEntrys, Bits(width = (AScaleNSlices * ScaleWidth * ReduceGroupSize).W))
    bank.suggestName("CUTE-A-Scale-Scratchpad-SRAM")
    
    //第0周期的数据
    val s0_bank_read_addr = ScaleControllerBankAddr
    val s0_bank_read_valid = read_go
    //第1周期的数据        
    val s1_bank_read_data = WireInit(0.U((AScaleNSlices * ScaleWidth * ReduceGroupSize).W))
    ...
    when(RegNext(read_go))
    {
        ...
        if (YJPDebugEnable)
        {
            printf("[ASSPD_Read]Bank: debug_s1_bank_addr = %d ,s1_bank_read_data = %x\n", debug_s1_bank_addr, s1_bank_read_data)
        }
    }
    //写数据
