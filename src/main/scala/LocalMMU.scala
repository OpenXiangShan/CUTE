...
class LocalMMU()(implicit p: Parameters) extends CuteModule{
    ...
    val outReqMaskBytes = PopCount(io.LastLevelCacheTLIO.Request.bits.RequestMask)
    io.perfProbe.rdReq := outReqFire && !outReqIsWr
    io.perfProbe.wrReq := outReqFire && outReqIsWr
    io.perfProbe.rdBytesReq := Mux(outReqFire && !outReqIsWr, outReqMaskBytes, 0.U).asUInt
    io.perfProbe.wrBytesReq := Mux(outReqFire && outReqIsWr, outReqMaskBytes, 0.U).asUInt
}
