...
class TaskController(implicit p: Parameters) extends BaseTaskController {
  ...
  val cmlOwnerValid = cmlLoadOwnerValid || cmlStoreOwnerValid
  ...
  io.CML_MicroTask_Config.MicroTaskEndReady := cmlOwnerValid
  ...
  val cmlDone = io.CML_MicroTask_Config.MicroTaskEndValid && io.CML_MicroTask_Config.MicroTaskEndReady
  val cmlLoadDone = cmlDone && cmlLoadOwnerValid
  val cmlStoreDone = cmlDone && cmlStoreOwnerValid
  ...
  when(io.CML_MicroTask_Config.MicroTaskEndValid) {
    assert(cmlOwnerValid, "TaskController: CML done without valid owner")
    assert(!(cmlLoadOwnerValid && cmlStoreOwnerValid), "TaskController: CML load/store owners overlap")
  }
  ...
  for (slotIdx <- 0 until WinDepth) {
    ...
    val slotFuReady = Mux(
      ...
      MuxLookup(slot.opKind.asUInt, true.B)(Seq(
        ...
        TaskCtrlOpKind.LoadC.asUInt -> (!fuCMLLoad.busy && !fuCMLStore.busy && io.CML_MicroTask_Config.MicroTaskReady),
        TaskCtrlOpKind.ZeroAcc.asUInt -> (!fuCMLLoad.busy && !fuCMLStore.busy && io.CML_MicroTask_Config.MicroTaskReady),
        TaskCtrlOpKind.ZeroTr.asUInt -> (!fuAML.busy && io.AML_MicroTask_Config.MicroTaskReady),
        TaskCtrlOpKind.Store.asUInt -> (!fuCMLLoad.busy && !fuCMLStore.busy && io.CML_MicroTask_Config.MicroTaskReady),
  ...
  private def loadByteCount(dim: UInt, widths: UInt): UInt = {
    MuxLookup(widths, dim << 2)(Seq(
      Bundles.MSew.e8 -> dim,
      Bundles.MSew.e16 -> (dim << 1),
      Bundles.MSew.e32 -> (dim << 2),
      Bundles.MSew.e4 -> (dim >> 1)
    ))
  }

  private def loadBeatCount(dim: UInt, widths: UInt): UInt = {
    ((loadByteCount(dim, widths) + (outsideDataWidthByte - 1).U) >> log2Ceil(outsideDataWidthByte))(MatrixRegMaxTensorDimBitSize - 1, 0)
  }
  ...
  when(issueFire) {
    switch(issueSlot.opKind) {
      is(TaskCtrlOpKind.LoadA) {
        ...
        assert(issueLsu.stride(5, 0) === 0.U, "TaskController load stride must be 64B aligned")
      ...
      is(TaskCtrlOpKind.Store) {
        ...
        assert(issueLsu.stride(5, 0) === 0.U, "TaskController store stride must be 64B aligned")
