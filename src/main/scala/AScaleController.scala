...
class AScaleController(implicit p: Parameters) extends CuteModule{
    val io = IO(new Bundle{
        ...
        val FromScarchPadIO = Flipped(new AScaleControlScaratchpadIO)
        val ConfigInfo = Flipped(new ADCMicroTaskConfigIO)
        val ScaleA = DecoupledIO(UInt((ScaleWidth*Matrix_M).W))
    ...
    when(state === s_mm_task){
        when(calculate_state === s_cal_init){
            ...
            }.elsewhen(io.ScaleA.valid && !io.ScaleA.ready && !io.ComputeGo && ScarchPadData.valid){
            ...
            }.elsewhen(io.ScaleA.valid && !io.ScaleA.ready && !io.ComputeGo && ScarchPadDataHoldValid)
            {
