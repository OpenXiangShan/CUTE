...
//BMatrixReg，用于暂存B矩阵的数据，供给TE模块使用
//B矩阵在DataController看来是一个只读的矩阵
//B矩阵需要支持滑动窗口，分Matrix_M个bank是合理的
//MatrixReg的功能是，根据输入的地址，输出数据

class BMatrixRegIO(implicit p: Parameters) extends CuteBundle{
    val FromDataController = new BDataControlMatrixRegIO
    val FromMemoryLoader = new BMemoryLoaderMatrixRegIO 
}

class BMatrixReg(scp_id:Int)(implicit p: Parameters) extends CuteModule{
    val io = IO(new Bundle{
        // val ConfigInfo = Flipped(DecoupledIO(new ConfigInfoIO))
        val MatrixRegIO = new BMatrixRegIO
    })
    ...
    val write_go = io.MatrixRegIO.FromMemoryLoader.BankAddr.zip(io.MatrixRegIO.FromMemoryLoader.Data).map{case (a,b) => a.valid && b.valid}
    val read_go = io.MatrixRegIO.FromDataController.BankAddr.valid && !io.MatrixRegIO.FromMemoryLoader.BankAddr.map(_.valid).reduce(_||_)  && !write_go.reduce(_||_) //写优先～
    //为输入信号赋ready
    ...
    val sram_banks = (0 until BMatrixRegNBanks) map { i =>

        //一个SeqMem就是一个SRAM，在一拍内完成读写，结果在下一拍输出，所以后头的代码里有s0，s1对不同阶段的流水数据进行分类，好区分每个周期的数据
        val bank = SyncReadMem(BMatrixRegBankNEntrys, Bits(width = (BMatrixRegEntryByteSize*8).W))
        bank.suggestName("CUTE-B-MatrixReg-SRAM")
        
        //第0周期的数据
        val s0_bank_read_addr = DataControllerBankAddr(i)
        val s0_bank_read_valid = read_go
        //第1周期的数据        
        val s1_bank_read_data = WireInit(0.U((BMatrixRegEntryByteSize*8).W))
        ...
        when(RegNext(read_go))
        {
            ...
            if (YJPDebugEnable)
            {
                printf("[BMatrixReg_Read(%d)]Bank(%d): debug_s1_bank_addr = %d ,s1_bank_read_data = %x\n", scp_id.U, i.U, debug_s1_bank_addr(0), s1_bank_read_data)
            }
        }
        //写数据
