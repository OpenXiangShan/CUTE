...
class MultiChannelsABMemLoader(label: String = "AML")(implicit p: Parameters) extends CuteModule{
    ...
    // sourceId 编码为 Cat(bankId, regAddr)，必须保证位宽足够
    val BankIdWidth = log2Ceil(ABMatrixRegNBanks)
    val RegAddrWidth = log2Ceil(ABMatrixRegBankNEntries)
    require(
      BankIdWidth + RegAddrWidth <= 64,
      s"RequestSourceID 位宽不足以容纳 Cat(bankId, regAddr): bankIdWidth=$BankIdWidth + regAddrWidth=$RegAddrWidth > 64"
    )
    println(s"[$label] BankIdWidth $BankIdWidth, RegAddrWidth $RegAddrWidth")
    ...
    class BankRespFifo(bankIdx: Int) {
        ...
        val curBaseAddr = WireInit(0.U(log2Ceil(ABMatrixRegBankNEntries).W))
        ...
        def enqFromResp(sourceId: UInt, respData: UInt): Unit = {
            ...
            q.io.enq.bits.baseAddr := sourceId(log2Ceil(ABMatrixRegBankNEntries) - 1, 0)
    ...
    def stepLoadWorking(): Unit = {
        ...
        for (i <- 0 until ABMatrixRegNBanks) {
            ...
            // sourceId 编码为 Cat(bankId, regAddr)，便于 Response 侧通过 router 路由到正确 channel
            val mregBaseAddr = Wire(UInt(RegAddrWidth.W))
            val mregBaseAddrCalc = (bankReqCount(i) * ReduceGroupSize.U) + current_K_index
            mregBaseAddr := mregBaseAddrCalc
            // assert(mregBaseAddrCalc === mregBaseAddr, cf"mregBaseAddrCalc !== mregBaseAddr, mregBaseAddrCalc $mregBaseAddrCalc, mregBaseAddr $mregBaseAddr")
            val bankId = i.U(BankIdWidth.W)
            request.bits.RequestSourceID := Cat(bankId, mregBaseAddr)
            ...
            when(io.LocalMMUIO.Response(i).fire) {
                ...
                // Router 已保证该 response 属于 bank i，直接取低位 regAddr
